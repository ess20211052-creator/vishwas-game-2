def on_overlap_tile(sprite11, location11):
    info.set_life(3)
    mySprite.ay = 200
    controller.move_sprite(mySprite, 100, 0)
    scene.camera_follow_sprite(mySprite)
    scene.set_background_image(img("""
        
        """))
    tiles.set_current_tilemap(tilemap("""
        層級7
        """))
    tiles.place_on_random_tile(mySprite, assets.tile("""
        我的貼圖25
        """))
scene.on_overlap_tile(SpriteKind.player,
    assets.tile("""
        我的貼圖21
        """),
    on_overlap_tile)

def on_overlap_tile2(sprite6, location6):
    info.set_life(3)
    mySprite.ay = 200
    controller.move_sprite(mySprite, 100, 0)
    scene.camera_follow_sprite(mySprite)
    scene.set_background_image(img("""
        
        """))
    tiles.set_current_tilemap(tilemap("""
        層級6
        """))
    tiles.place_on_random_tile(mySprite, assets.tile("""
        我的貼圖20
        """))
scene.on_overlap_tile(SpriteKind.player,
    assets.tile("""
        我的貼圖10
        """),
    on_overlap_tile2)

def on_overlap_tile3(sprite, location):
    controller.move_sprite(mySprite, 790, 0)
    mySprite.ay = 50
scene.on_overlap_tile(SpriteKind.player,
    assets.tile("""
        myTile6
        """),
    on_overlap_tile3)

def on_overlap_tile4(sprite5, location5):
    info.change_score_by(1)
    tiles.set_tile_at(location5, assets.tile("""
        transparency16
        """))
scene.on_overlap_tile(SpriteKind.player,
    sprites.dungeon.collectible_blue_crystal,
    on_overlap_tile4)

def on_a_pressed():
    if mySprite.vy == 0:
        mySprite.vy = -150
controller.A.on_event(ControllerButtonEvent.PRESSED, on_a_pressed)

def on_overlap_tile5(sprite12, location12):
    info.set_life(3)
    mySprite.ay = 200
    controller.move_sprite(mySprite, 100, 0)
    scene.camera_follow_sprite(mySprite)
    scene.set_background_image(img("""
        
        """))
    tiles.set_current_tilemap(tilemap("""
        層級4
        """))
    tiles.place_on_random_tile(mySprite, assets.tile("""
        我的貼圖15
        """))
scene.on_overlap_tile(SpriteKind.player,
    assets.tile("""
        我的貼圖8
        """),
    on_overlap_tile5)

def on_overlap_tile6(sprite2, location2):
    info.set_life(3)
    mySprite.ay = 200
    controller.move_sprite(mySprite, 100, 0)
    scene.set_background_image(img("""
        
        """))
    tiles.set_current_tilemap(tilemap("""
        level1
        """))
    tiles.place_on_random_tile(mySprite, assets.tile("""
        myTile7
        """))
scene.on_overlap_tile(SpriteKind.player,
    assets.tile("""
        myTile4
        """),
    on_overlap_tile6)

def on_overlap_tile7(sprite10, location10):
    info.set_life(3)
    mySprite.ay = 200
    controller.move_sprite(mySprite, 100, 0)
    scene.camera_follow_sprite(mySprite)
    scene.set_background_image(img("""
        
        """))
    tiles.set_current_tilemap(tilemap("""
        層級8
        """))
    tiles.place_on_random_tile(mySprite, assets.tile("""
        我的貼圖22
        """))
scene.on_overlap_tile(SpriteKind.player,
    assets.tile("""
        我的貼圖26
        """),
    on_overlap_tile7)

def on_overlap_tile8(sprite22, location22):
    info.set_life(3)
    mySprite.ay = 200
    controller.move_sprite(mySprite, 100, 0)
    scene.camera_follow_sprite(mySprite)
    scene.set_background_image(img("""
        
        """))
    tiles.set_current_tilemap(tilemap("""
        層級5
        """))
    tiles.place_on_random_tile(mySprite, assets.tile("""
        我的貼圖9
        """))
scene.on_overlap_tile(SpriteKind.player,
    assets.tile("""
        我的貼圖16
        """),
    on_overlap_tile8)

def on_overlap_tile9(sprite7, location7):
    info.set_life(3)
    mySprite.ay = 200
    controller.move_sprite(mySprite, 100, 0)
    scene.camera_follow_sprite(mySprite)
    tiles.set_current_tilemap(tilemap("""
        層級2
        """))
    scene.set_background_image(img("""
        
        """))
    tiles.place_on_random_tile(mySprite, assets.tile("""
        我的貼圖3
        """))
scene.on_overlap_tile(SpriteKind.player,
    assets.tile("""
        我的貼圖0
        """),
    on_overlap_tile9)

def on_overlap_tile10(sprite9, location9):
    info.set_life(3)
    mySprite.ay = 200
    controller.move_sprite(mySprite, 100, 0)
    scene.camera_follow_sprite(mySprite)
    scene.set_background_image(img("""
        
        """))
    tiles.set_current_tilemap(tilemap("""
        層級10
        """))
    tiles.place_on_random_tile(mySprite, assets.tile("""
        我的貼圖6
        """))
scene.on_overlap_tile(SpriteKind.player,
    assets.tile("""
        我的貼圖5
        """),
    on_overlap_tile10)

def on_overlap_tile11(sprite8, location8):
    tiles.set_tile_at(location8, assets.tile("""
        transparency16
        """))
scene.on_overlap_tile(SpriteKind.projectile,
    sprites.swamp.swamp_tile13,
    on_overlap_tile11)

def on_b_pressed():
    global projectile
    projectile = sprites.create_projectile_from_sprite(img("""
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . 2 2 . . . . . . .
            . . . . . . 3 1 1 3 . . . . . .
            . . . . . 2 1 1 1 1 2 . . . . .
            . . . . . 2 1 1 1 1 2 . . . . .
            . . . . . . 3 1 1 3 . . . . . .
            . . . . . . . 2 2 . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            """),
        mySprite,
        -50,
        0)
    projectile = sprites.create_projectile_from_sprite(img("""
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . 2 2 . . . . . . .
            . . . . . . 3 1 1 3 . . . . . .
            . . . . . 2 1 1 1 1 2 . . . . .
            . . . . . 2 1 1 1 1 2 . . . . .
            . . . . . . 3 1 1 3 . . . . . .
            . . . . . . . 2 2 . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            """),
        mySprite,
        50,
        0)
    projectile = sprites.create_projectile_from_sprite(img("""
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . 2 2 . . . . . . .
            . . . . . . 3 1 1 3 . . . . . .
            . . . . . 2 1 1 1 1 2 . . . . .
            . . . . . 2 1 1 1 1 2 . . . . .
            . . . . . . 3 1 1 3 . . . . . .
            . . . . . . . 2 2 . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            """),
        mySprite,
        0,
        -50)
    projectile = sprites.create_projectile_from_sprite(img("""
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . 2 2 . . . . . . .
            . . . . . . 3 1 1 3 . . . . . .
            . . . . . 2 1 1 1 1 2 . . . . .
            . . . . . 2 1 1 1 1 2 . . . . .
            . . . . . . 3 1 1 3 . . . . . .
            . . . . . . . 2 2 . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            . . . . . . . . . . . . . . . .
            """),
        mySprite,
        0,
        50)
controller.B.on_event(ControllerButtonEvent.PRESSED, on_b_pressed)

def on_overlap_tile12(sprite3, location3):
    info.set_life(3)
    mySprite.ay = 200
    controller.move_sprite(mySprite, 100, 0)
    scene.set_background_image(img("""
        
        """))
    tiles.set_current_tilemap(tilemap("""
        level3
        """))
    tiles.place_on_random_tile(mySprite, assets.tile("""
        myTile9
        """))
scene.on_overlap_tile(SpriteKind.player,
    assets.tile("""
        myTile8
        """),
    on_overlap_tile12)

def on_overlap_tile13(sprite32, location32):
    info.set_life(3)
    mySprite.ay = 200
    controller.move_sprite(mySprite, 100, 0)
    scene.camera_follow_sprite(mySprite)
    scene.set_background_image(img("""
        
        """))
    tiles.set_current_tilemap(tilemap("""
        層級9
        """))
    tiles.place_on_random_tile(mySprite, assets.tile("""
        myTile0
        """))
scene.on_overlap_tile(SpriteKind.player,
    assets.tile("""
        myTile
        """),
    on_overlap_tile13)

def on_life_zero():
    game.game_over(False)
info.on_life_zero(on_life_zero)

def on_overlap_tile14(sprite4, location4):
    info.set_life(3)
    mySprite.ay = 200
    controller.move_sprite(mySprite, 100, 0)
    scene.set_background_image(img("""
        
        """))
    tiles.set_current_tilemap(tilemap("""
        層級14
        """))
    tiles.place_on_random_tile(mySprite, assets.tile("""
        我的貼圖32
        """))
scene.on_overlap_tile(SpriteKind.player,
    assets.tile("""
        我的貼圖35
        """),
    on_overlap_tile14)

def on_overlap_tile15(sprite13, location13):
    info.set_life(3)
    mySprite.ay = 200
    controller.move_sprite(mySprite, 100, 0)
    scene.set_background_image(img("""
        
        """))
    tiles.set_current_tilemap(tilemap("""
        層級15
        """))
    tiles.place_on_random_tile(mySprite, assets.tile("""
        我的貼圖34
        """))
scene.on_overlap_tile(SpriteKind.player,
    assets.tile("""
        我的貼圖23
        """),
    on_overlap_tile15)

def on_overlap_tile16(sprite14, location14):
    info.set_life(3)
    mySprite.ay = 200
    controller.move_sprite(mySprite, 100, 0)
    scene.set_background_image(img("""
        
        """))
    tiles.set_current_tilemap(tilemap("""
        層級11
        """))
    tiles.place_on_random_tile(mySprite, assets.tile("""
        我的貼圖28
        """))
scene.on_overlap_tile(SpriteKind.player,
    assets.tile("""
        myTile10
        """),
    on_overlap_tile16)

def on_overlap_tile17(sprite42, location42):
    info.change_life_by(-3)
    tiles.set_tile_at(location42, assets.tile("""
        我的貼圖30
        """))
scene.on_overlap_tile(SpriteKind.player,
    assets.tile("""
        我的貼圖30
        """),
    on_overlap_tile17)

def on_overlap_tile18(sprite15, location15):
    game.game_over(True)
scene.on_overlap_tile(SpriteKind.player,
    assets.tile("""
        我的貼圖31
        """),
    on_overlap_tile18)

def on_overlap_tile19(sprite43, location43):
    info.change_life_by(-1)
    tiles.set_tile_at(location43, assets.tile("""
        transparency16
        """))
scene.on_overlap_tile(SpriteKind.player,
    sprites.swamp.swamp_tile13,
    on_overlap_tile19)

projectile: Sprite = None
mySprite: Sprite = None
info.set_life(3)
scene.set_background_image(img("""
    
    """))
tiles.set_current_tilemap(tilemap("""
    層級1
    """))
mySprite = sprites.create(img("""
        . . . . . . . . . . . . . . . .
        . . . . . f f f f f f . . . . .
        . . . f f e e e e f 2 f . . . .
        . . f f e e e e f 2 2 2 f . . .
        . . f e e e f f e e e e f . . .
        . . f f f f e e 2 2 2 2 e f . .
        . . f e 2 2 2 f f f f e 2 f . .
        . f f f f f f f e e e f f f . .
        . f f e 4 4 e b f 4 4 e e f . .
        . f e e 4 d 4 1 f d d e f f . .
        . . f e e e 4 d d d d f d d f .
        . . . . f e e 4 e e e f b b f .
        . . . . f 2 2 2 4 d d e b b f .
        . . . f f 4 4 4 e d d e b f . .
        . . . f f f f f f e e f f . . .
        . . . . f f . . . f f f . . . .
        """),
    SpriteKind.player)
mySprite.ay = 200
controller.move_sprite(mySprite, 100, 0)
scene.camera_follow_sprite(mySprite)
tiles.place_on_random_tile(mySprite, assets.tile("""
    我的貼圖
    """))
info.start_countdown(330)